...
### local-file:// Protocol (app icons)
Local file images (app icons) use a custom `local-file://` Electron protocol registered in `src/main/index.ts`.
**Use `readFileSync` + `Response`** — do NOT use `net.fetch('file://...')` which is unreliable in Electron protocol handlers.
Icon paths resolved via `resolveAppIcon()` in `src/main/services/projectDetector.ts` at app-link time, stored in electron-store.
